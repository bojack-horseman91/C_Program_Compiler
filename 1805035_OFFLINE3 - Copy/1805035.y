...
logic_expression : rel_expression 	{
	 	...
	 	$$ = $1;
	 	logfile2<<"Line "<<line_count<<": logic_expression : rel_expression"<<endl<<endl<<$$->getName()<<endl<<endl;
	 }
		 | rel_expression {
...
term :unary_expression{
     ...
     |  term MULOP unary_expression{
			...
			else if($3->getType() == "VOID" || $1->getType() == "VOID"){
				errorfile<<"Error at line "<<line_count<<" : Void function used in expression "<<endl<<endl; error_count++;
				logfile2<<"Error at line "<<line_count<<" : Void function used in expression "<<endl<<endl;
				$$->setType($1->getType());
			}		
			else if($1->getType() == "CONST_FLOAT" || $3->getType() == "CONST_FLOAT"){
				$$->setType("CONST_FLOAT");
			}	
			else {$$->setType("CONST_INT");}
			logfile2<<$$->getName()<<endl<<endl;		
